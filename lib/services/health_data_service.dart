...
class HealthDataService {
  ...
  Future<bool> requestPermissions() async {
    try {
      ...
      // 현재 권한 상태 확인
      bool? currentPermission = await _healthPlugin.hasPermissions(
        types,
        permissions: permissions,
      );
      developer.log('🔍 현재 권한 상태: $currentPermission');
      ...
      // 권한 요청 전 Health Connect 상태 확인 (Android)
      try {
        final isHealthConnectAvailable = await _healthPlugin
            .isHealthConnectAvailable();
        developer.log('🔍 Health Connect 사용 가능: $isHealthConnectAvailable');
      } catch (e) {
        developer.log('⚠️ Health Connect 상태 확인 실패: $e');
      }
    ...
    try {
      ...
      for (var workout in workouts) {
        ...
        try {
          ...
          savedCount++;
          developer.log(
            '✅ 저장 완료: ${workout['type']} (${workout['duration_minutes']}분, ${workout['calories'].toInt()}kcal)',
          );
        } catch (e) {
      ...
      developer.log(
        '📝 동기화 완료: $savedCount개 저장, $skippedCount개 건너뜀 (총 ${workouts.length}개)',
      );
